...
my $php_version = '';
if ($php_path =~ m{(?:^|/)lsphp(\d)(\d)(?:/|$)}) { $php_version = "$1.$2"; }
...
(function(){
  ...
  function initial(){var hash=(window.location.hash||'').substring(1);var valid=['overview','php','ssl','rewrite','logs','editor'];activate(valid.indexOf(hash)>=0?hash:'overview',false);}
...
info('Certificate',$ssl_cert || 'Not configured'); info('Private Key',$ssl_key || 'Not configured');
print "</div><a class='ols-ssl-action' href='ssl.cgi?vh=".&urlize($vh)."'>Manage SSL Certificates →</a></div></section></div>";
...
print "<div><span class='ols-label'>.htaccess</span><span class='ols-badge".($htaccess eq 'Enabled'?'':' off')."'>".$htaccess."</span></div></div></div></section></div>";
print "<div class='ols-panel' id='logs'><section class='ols-section'><h2>Domain Logs</h2><div class='ols-body'><p class='ols-note'>Latest 100 lines from logs belonging only to this virtual host.</p><details open><summary class='ols-log-title'>Access Log</summary><pre class='ols-log'>".&html_escape(read_tail($access_log,100))."</pre></details><br><details open><summary class='ols-log-title'>Error Log</summary><pre class='ols-log'>".&html_escape(read_tail($error_log,100))."</pre></details></div></section></div>";
print "<div class='ols-panel' id='editor'><section class='ols-section'><h2>Advanced Configuration</h2><div class='ols-body'><p class='ols-note'>Edit the complete virtual-host configuration. A backup is created before saving, the resulting server configuration is validated, and OpenLiteSpeed is restarted only after successful validation.</p><form method='post' action='config.cgi'><input type='hidden' name='vh' value='".&quote_escape($vh)."'><input type='hidden' name='action' value='save'><textarea class='ols-editor' name='config'>".&html_escape($content)."</textarea><br><br><input class='ols-save' type='submit' value='Save Configuration'></form></div></section></div>";
print "</div>";
&ui_print_footer('');
