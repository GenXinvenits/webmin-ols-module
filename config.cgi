...
print "<nav class='ols-tabs'><a href='#overview'>Overview</a><a href='#php'>PHP</a><a href='#ssl'>SSL</a><a href='#rewrite'>Rewrite</a><a href='#logs'>Logs</a><a href='#editor'>Advanced</a><a href='index.cgi'>← All Websites</a></nav>";
...
print "<section class='ols-section' id='overview'><h2>Website Overview</h2><div class='ols-body'><div class='ols-info'>";
...
print "<section class='ols-section' id='php'><h2>PHP Runtime</h2><div class='ols-body'><div class='ols-info'>";
info('Handler',$php_handler); info('Processor',$php_path); info('User',$php_user); info('Group',$php_group);
print "</div></div></section>";

print "<section class='ols-section' id='ssl'><h2>SSL / HTTPS</h2><div class='ols-body'><div class='ols-info'>";
...
print "<section class='ols-section' id='rewrite'><h2>Rewrite &amp; .htaccess</h2><div class='ols-body'><div class='ols-info'>";
...
print "<section class='ols-section' id='logs'><h2>Domain Logs</h2><div class='ols-body'><p class='ols-note'>Latest 100 lines from logs belonging only to this virtual host.</p>";
...
print "<details open><summary class='ols-log-title'>Error Log</summary><pre class='ols-log'>".&html_escape(read_tail($error_log,100))."</pre></details></div></section>";

print "<section class='ols-section' id='editor'><h2>Advanced Configuration</h2><div class='ols-body'><p class='ols-note'>Edit the complete virtual-host configuration. A backup is created before saving, the resulting server configuration is validated, and OpenLiteSpeed is restarted only after successful validation.</p>";
print "<form method='post' action='config.cgi'><input type='hidden' name='vh' value='".&quote_escape($vh)."'><input type='hidden' name='action' value='save'><textarea class='ols-editor' name='config'>".&html_escape($content)."</textarea><br><br><input class='ols-save' type='submit' value='Save Configuration'></form></div></section>";

print "<div class='ols-actions'><a class='ols-button' href='index.cgi'>← All Websites</a></div></div>";
&ui_print_footer('');
