...
print <<'HTML';
<style>
.ols-service{max-width:820px;margin:24px auto}.ols-card{border:1px solid var(--border-color,rgba(128,128,128,.25));border-radius:12px;overflow:hidden;background:var(--body-bg,transparent)}.ols-head{padding:24px 26px;border-bottom:1px solid var(--border-color,rgba(128,128,128,.18));display:flex;justify-content:space-between;align-items:flex-start;gap:20px}.ols-title{font-size:25px;font-weight:700;margin:0 0 5px}.ols-muted{font-size:13px;opacity:.68}.ols-badge{padding:5px 10px;border-radius:999px;font-size:10px;font-weight:700;white-space:nowrap}.ols-ok{background:rgba(40,167,69,.14);color:#43b56b}.ols-fail{background:rgba(220,53,69,.12);color:#d9534f}.ols-body{padding:24px 26px}.ols-result{padding:16px;border-radius:9px;border:1px solid var(--border-color,rgba(128,128,128,.18));margin-bottom:20px}.ols-result strong{display:block;font-size:15px;margin-bottom:5px}.ols-grid{display:grid;grid-template-columns:1fr;gap:12px}.ols-detail{padding:13px;border:1px solid var(--border-color,rgba(128,128,128,.18));border-radius:8px}.ols-label{font-size:10px;text-transform:uppercase;letter-spacing:.05em;opacity:.55;margin-bottom:4px}.ols-value{font-size:13px;font-weight:600}.ols-status-running{color:#43b56b}.ols-status-stopped{color:#d9534f}.ols-output{margin-top:16px}.ols-output pre{padding:14px;border-radius:8px;overflow:auto;font-size:11px;background:rgba(128,128,128,.08)}.ols-actions{margin-top:22px;display:flex;gap:8px;flex-wrap:wrap}.ols-btn{display:inline-block;padding:8px 13px;border:1px solid var(--border-color,rgba(128,128,128,.25));border-radius:7px;text-decoration:none;font-weight:600;font-size:12px}.ols-btn-primary{background:rgba(128,128,128,.1)}
</style>
HTML
...
my $status_class = $running ? 'ols-status-running' : 'ols-status-stopped';
my $status_dot = $running ? '●' : '●';
print "<div class='ols-grid'><div class='ols-detail'><div class='ols-label'>Current status</div><div class='ols-value $status_class'>$status_dot " . &html_escape($status) . "</div></div></div>";
