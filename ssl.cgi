...
if ($in{'action'} eq 'selfsigned') {
    ...
    my $version = selfsigned_version($archive_dir);
...
if (!$certbot_installed) {
    print "<p class='ols-note'>Certbot is not installed. Use the SSL Dependencies button above to install certbot before requesting a Let's Encrypt certificate.</p>";
} else {
...
if ($certbot_installed) {
    print "<button class='ols-btn primary' id='ssl-renew' type='submit' name='action' value='renew' formnovalidate>Renew Let's Encrypt Certificate</button>";
}
...
(function(){
  ...
  var renew=document.getElementById('ssl-renew');
  ...
  function sync(){
    ...
    if(renew)renew.style.display=le?'inline-block':'none';
