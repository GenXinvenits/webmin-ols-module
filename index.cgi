...
print "<div class='ols-section-head'><h2>Websites</h2><a class='ols-add-domain' href='domains.cgi'>Add / Remove Domains</a></div><div class='ols-list'><div class='ols-row ols-head'><div>Domain</div><div>Document Root</div><div>Status</div><div>Action</div></div>";
...
print "</div>";
if($total>$per_page){print "<div class='ols-pagination'><div class='ols-meta'>Showing ".($start+1)."–".($end+1)." of $total domains</div><div class='ols-pages'>";print "<a class='ols-page' href='index.cgi?page=".($page-1)."'>‹</a>" if $page>1;for(my $p=1;$p<=$pages;$p++){next if $pages>8&&$p!=1&&$p!=$pages&&abs($p-$page)>1;print "<a class='ols-page".($p==$page?' current':'')."' href='index.cgi?page=$p'>$p</a>";}print "<a class='ols-page' href='index.cgi?page=".($page+1)."'>›</a>" if $page<$pages;print "</div></div>";}
print "</div>";
&ui_print_footer('');
