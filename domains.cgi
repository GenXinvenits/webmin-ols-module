...
print <<'HTML';
<script>
(function(){ var domain=document.getElementById('ols-domain'),suffix=document.getElementById('ols-alias-suffix'); function updateSuffix(){if(!domain||!suffix)return;var v=domain.value.trim().replace(/^https?:\/\//i,'').replace(/\/.*$/,'');suffix.textContent=v?'.'+v:'.example.com';} if(domain)domain.addEventListener('input',updateSuffix);updateSuffix(); })();
</script>
HTML
