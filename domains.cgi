...
# Match the index page domain ordering and pagination.
my $sort=$in{'sort'}||'newest';
$sort='newest' unless $sort =~ /^(?:newest|oldest|az|za)$/;
if ($sort eq 'az' || $sort eq 'za') {
    @domains = sort { lc($a) cmp lc($b) || $a cmp $b } @domains;
    @domains = reverse @domains if $sort eq 'za';
} else {
    @domains = sort {
        my $am=(stat("$domain_base/$a/conf/vhconf.conf"))[9] || (stat("$domain_base/$a"))[9] || 0;
        my $bm=(stat("$domain_base/$b/conf/vhconf.conf"))[9] || (stat("$domain_base/$b"))[9] || 0;
...
print "<section class='ols-card'><div class='ols-section-head'><h2>Registered Domains</h2><div class='ols-sort'><label for='domain-sort'>Sort by</label><select id='domain-sort' onchange=\"window.location='domains.cgi?sort='+encodeURIComponent(this.value)\"><option value='newest'".($sort eq 'newest'?' selected':'').">Newest first</option><option value='oldest'".($sort eq 'oldest'?' selected':'').">Oldest first</option><option value='az'".($sort eq 'az'?' selected':'').">Alphabetical (A–Z)</option><option value='za'".($sort eq 'za'?' selected':'').">Alphabetical (Z–A)</option></select></div></div><div class='ols-body'><p class='ols-note'>Removing a domain unregisters its OpenLiteSpeed configuration and listener mappings, and deletes its domain directory and SSL certificate directories.</p><div class='ols-list'><div class='ols-row ols-head'><div>Domain</div><div>Virtual Host</div><div>Action</div></div>";
if (!@domains) { print "<div class='ols-body ols-note'>No domains registered.</div>"; }
else {
    for (my $i=$start; $i<=$end; $i++) {
        my $d=$domains[$i];
        print "<div class='ols-row'><div class='ols-domain'>".&html_escape($d)."</div><div>".&html_escape("$domain_base/$d")."</div><div><form method='post' action='domains.cgi' onsubmit=\"return confirm('WARNING: Delete $d permanently? This removes the domain files, configuration and SSL certificates and cannot be undone.');\"><input type='hidden' name='action' value='remove'><input type='hidden' name='domain' value='".&quote_escape($d)."'><input type='hidden' name='confirm_remove' value='yes'><button class='ols-btn ols-danger' type='submit'>Remove</button></form></div></div>";
    }
}
print "</div>";
if ($total > $per_page) {
    my $base='domains.cgi?sort='.urlize($sort).'&page=';
    print "<div class='ols-pagination'><div class='ols-note'>Showing ".($start+1)."–".($end+1)." of $total domains</div><div class='ols-pages'>";
    print "<a class='ols-page' href='".$base.($page-1)."'>‹</a>" if $page>1;
    for (my $p=1; $p<=$pages; $p++) {
        next if $pages>8 && $p!=1 && $p!=$pages && abs($p-$page)>1;
        print "<a class='ols-page".($p==$page?' current':'')."' href='".$base.$p."'>$p</a>";
    }
    print "<a class='ols-page' href='".$base.($page+1)."'>›</a>" if $page<$pages;
    print "</div></div>";
}
