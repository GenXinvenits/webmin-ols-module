...
sub remove_vhost_block {
    my ($text, $vh) = @_;
    my @lines = split(/(?<=\n)/, $text, -1);
    my @out; my $skip = 0; my $depth = 0; my $found = 0;
    for my $line (@lines) {
        if (!$skip && $line =~ /^\s*virtualhost\s+\Q$vh\E\s*\{/i) {
            $skip = 1; $found = 1; $depth = 0;
        }
        if ($skip) {
            my $opens = () = $line =~ /\{/g;
            my $closes = () = $line =~ /\}/g;
...
if ($in{'action'} eq 'add') {
    ...
    my $alias_prefixes = lc($in{'alias_prefixes'} || ''); $alias_prefixes =~ s/^\s+|\s+$//g;
    my $aliases = '';
    ...
    if (!valid_domain($domain)) { $error = 'Enter a valid domain name, for example example.com.'; }
        ...
        $aliases = build_aliases($domain, $alias_prefixes);
        ...
        if (!$paths_ok) { $error = "Unable to create the domain directories: $@"; }
            ...
            if (!$error) {
                ...
                if ($ok) { $message = "Domain $domain was added successfully."; }
                else { unlink($vh_conf); $error = $out; }
            }
        }
    }
}
    ...
    if (!$domain || !grep { $_ eq $domain } vhost_names($content)) { $error = 'The selected domain is not registered.'; }
        ...
        if ($found) {
            ...
            if ($ok) {
                ...
                $message = "Domain $domain was removed from OpenLiteSpeed. Website files were left untouched.";
            } else { $error = $out; }
...
print "<section class='ols-card'><h2>Registered Domains</h2><div class='ols-body'><p class='ols-note'>Removing a domain unregisters its OpenLiteSpeed configuration and listener mappings. Website files are preserved.</p><div class='ols-list'>";
...
<script>
(function(){
    var domain = document.getElementById('ols-domain');
    var suffix = document.getElementById('ols-alias-suffix');
    if (!domain || !suffix) return;
    function updateSuffix(){
        var value = domain.value.trim().replace(/^https?:\/\//i,'').replace(/\/.*$/,'');
        suffix.textContent = value ? '.' + value : '.example.com';
    }
    domain.addEventListener('input', updateSuffix);
    updateSuffix();
})();
</script>
HTML
